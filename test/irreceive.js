...
describe('IRReceive', function() {
  ...
  describe('[child process listening for input] upon receiving input', function() {
    ...
    xit('should execute all callbacks matching key and remote', function() {
      assert.fail();
    });

    xit('should execute callbacks matching just the key if no remote was specified', function() {
      assert.fail();
    });

    xit('should execute all callbacks set with no key and no remote specified', function() {
      assert.fail();
    });

    xit('should execute the callback(s) each time the input is received, if no throttle was set', function() {
      assert.fail();
    });

    xit('should execute the callback(s) just once until the key is held down, if throttle was set to 0', function() {
      assert.fail();
    });

    xit('should execute the callback(s) once every Xms while the key is held down, if throttle was set to a positive integer', function() {
      assert.fail();
    });
